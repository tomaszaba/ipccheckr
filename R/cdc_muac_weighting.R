...
#'
#' `cdc_classify_wasting()` is used a helper inside [cdc_apply_age_weighting()] to
...
#'
#'
cdc_classify_wasting <- function(muac, edema = NULL) {
  if (!is.null(edema)) {
...
#'
#' `cdc_apply_age_weighting()` does that. It takes the proportion of children
...
#'
#'
cdc_apply_age_weighting <- function(muac, age,
  ...
  if (!is.null(edema)) {
    ### Define cases ----
    nut_status <- cdc_classify_wasting(muac, edema)
    ...
    ### Define cases ----
    nut_status <- cdc_classify_wasting(muac)
